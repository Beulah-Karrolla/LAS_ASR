...
class Trainer(object):
    ...
    def asr(self, model, logger):
        ...
        for epoch in range(self.args.epochs_done+1, self.args.nepochs+1):
            ...
            for speechB, textB, logitLens in tqdm(self.loader):
                model.train()
                step += 1
                lens_norm = [1.*(x/speechB.size(1)) for x in logitLens]
                ...
                speechB, textB, logitLens = load2gpu(speechB, self.device), load2gpu(textB, self.device), load2gpu(logitLens, self.device)
                loss = model(speechB, logitLens, textB)
                if torch.isnan(loss):
